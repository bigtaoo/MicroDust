...
namespace ET.Client
{
    [UIEvent(UIType.MicroDustBattleSummary)]
    public class MicroDustBattleSummaryUIEvent : AUIEvent
    {
        public override async ETTask<UI> OnCreate(UIComponent uiComponent, UILayer uiLayer)
        {
            await ETTask.CompletedTask;
            string assetsName = $"Assets/Bundles/UI/MicroDust/Battle/{UIType.MicroDustBattleSummary}.prefab";
            GameObject bundleGameObject = await uiComponent.Scene().GetComponent<ResourcesLoaderComponent>().LoadAssetAsync<GameObject>(assetsName);
            GameObject gameObject = UnityEngine.Object.Instantiate(bundleGameObject, uiComponent.UIGlobalComponent.GetLayer((int)uiLayer));
            UI ui = uiComponent.AddChild<UI, string, GameObject>(UIType.MicroDustBattleSummary, gameObject);
            ui.AddComponent<MicroDustBattleSummaryUIComponent>();

            //var uiType = UIType.MicroDustBattleSummary;
