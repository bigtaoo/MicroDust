...
namespace ET.Client
{
    [UIEvent(UIType.MicroDustSelectHero)]
    public class MicroDustSelectHeroUIEvent : AUIEvent
    {
        public override async ETTask<UI> OnCreate(UIComponent uiComponent, UILayer uiLayer)
        {
            await ETTask.CompletedTask;
            string assetsName = $"Assets/Bundles/UI/MicroDust/MajorCity/{UIType.MicroDustSelectHero}.prefab";
            GameObject bundleGameObject = await uiComponent.Scene().GetComponent<ResourcesLoaderComponent>().LoadAssetAsync<GameObject>(assetsName);
            GameObject gameObject = UnityEngine.Object.Instantiate(bundleGameObject, uiComponent.UIGlobalComponent.GetLayer((int)uiLayer));
            UI ui = uiComponent.AddChild<UI, string, GameObject>(UIType.MicroDustSelectHero, gameObject);
            ui.AddComponent<MicroDustSelectHeroUIComponent>();

            //var uiType = UIType.MicroDustSelectHero;
